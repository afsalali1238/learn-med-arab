...
export function VocabBankModal({ open, onOpenChange, entries, onAdd, onRemove }: Props) {
  ...
  const submit = () => {
    ...
    onAdd({ arabic: arabic.trim(), transliteration: translit.trim(), note: note.trim() || undefined });
    setArabic("");
    setTranslit("");
    setNote("");
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] max-w-2xl overflow-hidden">
        <DialogHeader>
          <DialogTitle>Clinical Vocabulary Bank</DialogTitle>
          <DialogDescription>
