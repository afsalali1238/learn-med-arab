...
// 1. General Vocab
const bodyPartsList = ['Blood', 'Skin', 'Liver', 'Kidney', 'Urine', 'Heart', 'Ear', 'Milk', 'Food', 'Face', 'Joint', 'Conjunctiva', 'Cerebrum', 'Brain', 'Tongue', 'Breast'];
...
// 2. Symptoms
const chronicList = ['Cancer', 'Paralysis', 'Anemia', 'Coma', 'Cataract', 'Psoriasis', 'Warts', 'Hypoglycemia', 'Hyperglycemia', 'Hepatitis', 'Peptic Ulcer', 'Hernia', 'Syphilis', 'Gonorrhea', 'Cholera', 'Scabies', 'Gangrene'];
const severeList = ['Toxicity / Poisoning', 'Bleeding', 'Spasm / Cramp', 'Swelling', 'Small Pox', 'Measles', 'Whooping Cough'];
...
for (let i = 0; i < WEEKS.length; i++) {
  ...
  if (week.vocabTables && week.vocabTables.length > 0) {
    ...
    for (const term of newTerms) {
      const engTerm = term[0].toLowerCase().trim().replace(/ \/ .*/, '');
      const exists = existingRows.some(row => row[0].toLowerCase().trim().replace(/ \/ .*/, '').includes(engTerm) || engTerm.includes(row[0].toLowerCase().trim().replace(/ \/ .*/, '')));
      if (!exists) {
